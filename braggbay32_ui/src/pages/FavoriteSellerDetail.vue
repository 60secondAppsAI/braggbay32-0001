					...
					<tr> 
						<td style="width: 100%;"></td>
				        	<td style="width: 150px;">
				      			<base-button class="btn btn-primary" @click="updateFavoriteSeller()">Save</base-button>
				        	</td>
				        	<td style="width: 150px;">
				        	<td style="width: 150px;">
								...
								<div >
								  <div class="container">
												    <base-button class="btn btn-primary" @click="modal2FavoriteSellers = true">Add</base-button>
								              <modal :show.sync="modal2FavoriteSellers">
          ...
          <a-modal :footer="null" :visible="userPermissionDialog" :closable="false" class="centered-modal" v-if="userPermissionDialog">
            <div style="text-align: center; padding: 20px;">
              ...
              <button @click="handlePermissionDialogClose">OK</button>
										...
										<tr> 
					<td class="detail_view_column_2">Favorite Seller Id</td>
										    <td class="detail_view_column_1">
							<div class="detail_view_input" style="display: inline-flex;">
																		<input label="FavoriteSellerId" type="text" placeholder="Enter FavoriteSellerId" v-model="favoriteSellerDetails.favoriteSellerId"></input>
																		</div>
						</td>
														</tr>
...
export default {
  ...
  data() {
    return {
      favoriteSellerToAdd: {},
      modal2FavoriteSellers: false,
      favoriteSellerDetails: null,
  ...
  methods: {
    ...
    async handleAddSubmitted() {
      this.modal2FavoriteSellers = false;
    ...
    async updateFavoriteSeller() {

      const jsonData = JSON.stringify(this.favoriteSellerDetails);
      const res = await FavoriteSellerService.update(jsonData);
...
//        this.getAllFavoriteSellers();
    ...
    async getFavoriteSellerDetails() {
      try {
			let response = await FavoriteSellerService.get(this.favoriteSellerId);
			this.favoriteSellerDetails = response.data;

        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <div>
                  <base-button class="btn btn-primary" @click="modalWishlistItems = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalWishlistItems">
                <template slot="header">
                  <h5 class="modal-title" id="exampleModalLabel">Add WishlistItem</h5>
                </template>
                <div>
                  <form @submit.prevent>
  <base-input label="WishlistItemId" type="text" placeholder="Enter WishlistItemId" v-model="wishlistItemToAdd.wishlistItemId"></base-input>
                  </form>
                </div>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="wishlistItems" :row-key="record => record.WishlistItemId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <WishlistItemPictureView :wishlistItems="wishlistItems" />
          </div>
...
import WishlistItemPictureView from './WishlistItemPictureView.vue';
...
export default {
  ...
  components: {
    ...
    Pagination,
    WishlistItemPictureView
  },

  data() {
    return {
      modalWishlistItems: false,
        isTableView: true,
      ...
      columns: [
		...
		dataIndex: 'wishlistItemId',
          visible: true,
          scopedSlots: { customRender: 'wishlistItemId' },
          sorter: true
          //sorter: (a, b) => a.wishlistItemId - b.wishlistItemId,
          //sorter: (a, b) => a.wishlistItemId.localeCompare(b.wishlistItemId),
        },
      ],
      pagination: {
        current: 1,
        ...
        showTotal: (total) => `Total ${total} wishlistItems`,
      },
      ...
      sortBy: 'wishlistItemId',           // Column to sort by
  ...
  methods: {
    ...
    async renderWishlistItemsTable() {
      ...
      let wishlistItemsTableData = [];
      for (let i = 0; i < this.wishlistItems.length; i++) {
        wishlistItemsTableData.push({
          id: i,
          wishlistItemId: this.wishlistItems[i].wishlistItemId,
          year: this.wishlistItems[i].year,
          date: this.wishlistItems[i].date,
          competitionId: this.wishlistItems[i].competitionId,
          wishlistItemId: this.wishlistItems[i].wishlistItemId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-wishlist-items',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForWishlistItemsChanged', this.searchQuery);
		//this.renderWishlistItemsTable();
    },
	...
	async handleAddSubmitted() {
      this.modalWishlistItems = false;

      const currentDate = new Date().getTime();
      this.wishlistItemToAdd.created = currentDate;

      const jsonData = JSON.stringify(this.wishlistItemToAdd);
      console.log(jsonData);
      
      const res = await WishlistItemService.addWishlistItem(jsonData);
